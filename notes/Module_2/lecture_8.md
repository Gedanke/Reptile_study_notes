...
Python 中 ```re``` 库提供了正则表达式的实现，下面就来了解它的一些常用方法。
...
在匹配结果 result 中有两个方法：group 方法输出匹配的内容，结果是 Hello 123 4567 World_This，这恰好是正则表达式规则所匹配的内容。```span``` 方法输出匹配的范围，结果是 (0, 25)，这就是匹配到的结果字符串在原字符串中的位置范围。
...
刚刚我们用 ```match``` 方法得到了匹配的字符串，如果我们想从字符串中提取一部分内容的话，可以使用 () 括号待提取的子字符串括起来。() 实际上标记了一个子表达式的开始和结束位置，被标记的每个子表达式会依次对应每一个分组，调用 group 方法传入分组的索引即可获取提取的结果。
